...
const BreadcrumbPurposePage = Loadable({
  loader: () => import('../components/breadcrumbs/BreadcrumbPurposePage'),
  loading: () => null,
});
...
class PurposeLayout extends Component {
  constructor(props) {
    super(props);
    this.state = {
      document: 'purpose',
  ...
  render() {
    //console.log('|--> PurposeLayout render')
    const { t } = this.props;

    return (
      <MainContainer
        breadcrumb={BreadcrumbPurposePage}
        footer={Footer}
      >
        <Helmet>
          <title>{t('footer.purpose')} | {t('projectName')}</title>
        </Helmet>
        {
