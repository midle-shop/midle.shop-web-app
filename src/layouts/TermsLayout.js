...
const BreadcrumbTermsPage = Loadable({
  loader: () => import('../components/breadcrumbs/BreadcrumbTermsPage'),
  loading: () => null,
});
...
class TermsLayout extends Component {
  ...
  render() {
    //console.log('|--> TermsLayout render')
    const { t } = this.props;

    return (
      <MainContainer
        breadcrumb={BreadcrumbTermsPage}
        footer={Footer}
      >
        <Helmet>
          <title>{t('footer.termsOfUse')} - {t('footer.legal')} | {t('projectName')}</title>
        </Helmet>
        {
