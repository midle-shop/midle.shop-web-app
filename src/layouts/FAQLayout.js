...
const BreadcrumbFAQPage = Loadable({
  loader: () => import('../components/breadcrumbs/BreadcrumbFAQPage'),
  loading: () => null,
});
...
class FAQLayout extends Component {
  ...
  render() {
    //console.log('|--> FAQLayout render')
    const { t } = this.props;

    return (
      <MainContainer
        breadcrumb={BreadcrumbFAQPage}
        footer={Footer}
      >
        <Helmet>
          <title>{t('menu.faq')} | {t('projectName')}</title>
        </Helmet>
        {
