...
const BreadcrumbCookiesPage = (props) => {
  //console.log('|--> BreadcrumbCookiesPage render')
  ...
  return(
    <Breadcrumb style={{marginTop: '1em', marginBottom: '1em',}} size={props.size ? props.size : undefined}>
      ...
      <Breadcrumb.Divider />
      <Breadcrumb.Section as={Link} to={ml.url('/legal')}>{t('footer.legal')}</Breadcrumb.Section>
      <Breadcrumb.Divider />
      <Breadcrumb.Section as={Link} to={ml.url('/legal/privacy')}>{t('footer.privacyPolicy')}</Breadcrumb.Section>
      <Breadcrumb.Divider />
      <Breadcrumb.Section active>{t('footer.useOfCookies')}</Breadcrumb.Section>
    </Breadcrumb>
  );
}
...
export default hot(module)(
  connect(mapStateToPropsBC)(
    translate('main')(BreadcrumbCookiesPage)
  )
);
